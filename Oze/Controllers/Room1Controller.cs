...
{
    public class Room1Controller : BaseController
    {
        // GET: Rooms
        [HttpGet]
        ...
        public ActionResult UpdateStatusRoom(int id,string status )
        {
            int result = new RoomService().UpdateStatusRoom(id,status);
            return Json(new { result = result }, JsonRequestBehavior.AllowGet);
        }
        public ActionResult UpdateStatusRoom1(int id, string status)
        {
            int result = new RoomService().UpdateStatusRoom1(id, status);
            return Json(new { result = result }, JsonRequestBehavior.AllowGet);
        }
